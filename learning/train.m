...
for t = 1:iter
  ...
  for i = 1:length(neg)
    ...
    if isfield(neg,'force')
      bbox.xy = [neg(i).x1 neg(i).y1 neg(i).x2 neg(i).y2];
      bbox.force = neg(i).force;
      area = (bbox.xy(:,3)-bbox.xy(:,1)+1).*(bbox.xy(:,4)-bbox.xy(:,2)+1);
      area = area(bbox.force==1);
      ...
      [box,model,loss] = detect(im, model, -1, bbox, overlap, i, -1, loss);
    else
      im = imread(neg(i).im);
      [box,model,loss] = detect(im, model, -1, [], 0, i, -1,loss);
    end
    fprintf(' #cache+%d=%d/%d, #sv=%d, LB=%.4f, loss=%.4f',numel(box),qp.n,nmax,sum(qp.sv),qp.obj,loss);
...
%     globals;
%     clf; subplot(1,2,1); showboxes(im, bbox, PROX1Popts.color, PROX1Popts.linestyle);
% 		subplot(1,2,2); showboxes(im, box, PROX1Popts.color, PROX1Popts.linestyle);
	end
end
