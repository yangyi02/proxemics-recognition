...
for rlevel = levels
  ...
  for c  = randperm(length(model.components))
    ...
    for i = 1:length(X)
      ...
      y = Y(i);
      m = Im(y,x);
%       [boxes(cnt).xy,boxes(cnt).m,boxes(cnt).v,ex] = backtrack(x,y,m,parts,pyra,ex,latent || write);
...
%     if write && ~latent && ~isempty(X) && qp.n < length(qp.a)
    if write && ~poslatent && ~isempty(X) && qp.n < length(qp.a)
...
if latent && ~neglatent && ~isempty(boxes)
%   boxes = boxes(end);
  boxes = boxes(end,:);
  if write
    qp_write(ex);
